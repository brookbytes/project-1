...
export default function Home() {
  ...
  useEffect(() => {
    const fetchAnimeData = async () => {
      try {
        ...
        setCurrentlyAiring(dataAiring.data.slice(0, 10));
        setPopularAnime(dataPopular.data.slice(0, 10));
        setLatestCompleted(dataCompleted.data.slice(0, 10));
      } catch (error) {
  ...
  const animeSections = [
    ...
    { title: "Latest Completed", data: latestCompleted },
  ];
